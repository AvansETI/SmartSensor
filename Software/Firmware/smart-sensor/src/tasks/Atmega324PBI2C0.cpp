...
{ // TODO: Use millis to check when the I2C is stuck.
    ...
    case I2CState::CHECK:
    {
        ...
        if (this->ready() || *command == I2CCommand::STOP)
        { // When the I2C operation has finished, proceed to check the status.
    ...
    { // Variable used inside a switch are known to all cases, so limiting the scope here.
        if (millis - this->timer > 60000)
        {
            ...
            if (*command != I2CCommand::STOP)
            {
                ...
                { // The status is correct, we are finished!
                    if (*command == I2CCommand::READ_ACK || *command == I2CCommand::READ_NACK)
                    { // Get the data!
                ...
                { // The status is not correct, empty the queue
...
bool Atmega324PBI2C0::wait(uint8_t status)
{ // TW_START
    while (!(TWCR0 & (1 << TWINT)))
        ;
    return (TWSR0 & 0xF8) == status;
...
void Atmega324PBI2C0::repeatedStart()
{ // Same as start different result
...
{
    if (this->state != I2CState::WAITING)
        return false; // Do nothing when already busy for another function!
